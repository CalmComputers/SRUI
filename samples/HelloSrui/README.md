...
|---|---|---|
| Srui.Net.dll | Widget classes, app shell, P/Invoke layer | UI |
| srui_ffi.dll | Core engine, SDL host, speech — the C ABI | UI (native) |
...
A UI-only application ships the first four; an audio-only one ships the last three.
...
- Copy the native DLLs beside the exe (`<None>` items with `CopyToOutputDirectory`). The P/Invoke layer loads them by name from the application directory.
- Target `net10.0`.
