...
static ma_decoding_backend_vtable ma_gDecodingBackendVTable_libopus =
{
    ...
    /* SRUI: onInitFile disabled — it reaches op_open_file, whose ANSI
     * fopen breaks non-ASCII paths on Windows. With it NULL, every
     * consumer takes the callbacks path through miniaudio's VFS, which
     * handles UTF-8 (PATCHES.md). */
    NULL,
    NULL, /* onInitFileW() */
    NULL, /* onInitMemory() */
