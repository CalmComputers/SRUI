...
- Inside OnInput, mutate only your own widget's state and label; anything that touches other widgets or the tree (opening dialogs, removing widgets) must be deferred via Post/PostChanged so it runs at drain time, outside dispatch. The built-ins model this.
